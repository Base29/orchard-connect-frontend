...
export default function NewsPage() {
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast("Failed to retrieve news articles.", "error");
      }
    } catch (err) {
  ...
  // Fetch older news pages (Infinite Scroll)
  ...
  // Strips HTML tags for rendering textual snippets
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 flex flex-col items-center justify-center p-6 transition-colors duration-200">
        ...
        <p className="text-sm font-light text-slate-400 dark:text-zinc-500">Syncing news feed...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-neutral-100 flex flex-col font-sans transition-colors duration-200">
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <aside className="lg:col-span-3 space-y-6">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-neutral-200/60 dark:border-zinc-800/80 p-5 space-y-5 shadow-sm">
            ...
            <nav className="flex flex-col gap-1 text-sm font-medium">
              ...
              <Link href="/dashboard/news" className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-neutral-50 dark:bg-zinc-800 text-slate-900 dark:text-neutral-100 transition-colors">
                📰 Orchard News
              </Link>
              ...
              <Link href="/dashboard/polls" className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-neutral-50 dark:hover:bg-zinc-855 text-slate-600 dark:text-zinc-400 transition-all">
                📊 Polls
              </Link>
            </nav>
        ...
        <main className="lg:col-span-9 space-y-6">
          ...
          {/* Header Row */}
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Orchard News</h1>
            <p className="text-xs font-light text-slate-400 dark:text-zinc-400">
              Stay up to date with the latest official news, newsletters, and announcements published by society directors.
            </p>
          </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className="text-xs text-slate-400 dark:text-zinc-400 font-light">Retrieving news articles...</p>
            </div>
          ) : newsList.length === 0 ? (
            <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-neutral-200/60 dark:border-zinc-800/80 py-20 text-center shadow-sm">
              <p className="text-slate-400 dark:text-zinc-400 text-sm font-light">No news articles published yet.</p>
            </div>
          ) : (
            <div className="space-y-6">
                  ...
                  <div className="flex-1 flex flex-col justify-between space-y-3">
                    <div className="space-y-2">
                      ...
                      <Link href={`/dashboard/news/${news.id}`}>
                        <h2 className="text-base md:text-lg font-bold text-slate-800 dark:text-zinc-100 hover:text-emerald-500 transition-colors leading-tight cursor-pointer">
                          {news.title}
                        </h2>
                      </Link>
              ...
              {hasMore && (
                <div ref={observerTarget} className="flex flex-col items-center justify-center py-6 gap-2">
                  ...
                  <p className="text-[10px] font-light text-slate-500 dark:text-zinc-400">Loading older updates...</p>
                </div>
              )}
            </div>
          )}
